...
drop proc sp_rep_ahorros_210150
go


create proc sp_rep_ahorros_210150(
...
--  cd_numero           int)
  ...
  truncate table tmp_rep_liqaho
  truncate table tmp_rep_credeb
--INICIALIZO VARIABLES
select @w_sp_name        = 'sp_rep_ahorros_210150',
       @w_tasa_base      = '0',
...
if exists (select 1 from anda_reporte..rep_cuadre_pasivas
           where  rf_prod_cobis = @w_prod_cobis and rf_fecha_carga=@i_fecha_desde and rf_cuenta='210150')
begin
...
select ah_cuenta,      ah_cliente,     ah_nombre,
       ah_fecha_aper,  ah_cta_banco,   ROUND(ah_12h + ah_24h + ah_48h + ah_remesas,2),
...
and    ah_estado <> "C"
and    ROUND(ah_12h + ah_24h + ah_48h + ah_remesas,2) > 0
...
declare cursor_ahorros_210150 cursor for
...
open cursor_ahorros_210150

fetch cursor_ahorros_210150 into  
...
begin   
   ...
   select @w_saldo_ah = isnull(@w_saldo, 0.00)


   -- CARGA INICIAL DE MOVIMIENTOS
   truncate table tmp_rep_credeb

   insert into tmp_rep_credeb
   ...
   select @w_cuota_capital = isnull(@w_saldo_ah,0),
          ...
          @w_substr1       = null,
          @w_substr2       = null,
          @w_producto      = 'PRODUCTO AHORROS CHEQUES'
...
close cursor_ahorros_210150
deallocate cursor cursor_ahorros_210150
