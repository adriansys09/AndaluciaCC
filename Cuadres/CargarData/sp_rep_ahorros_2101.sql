...
drop proc sp_rep_ahorros_2101
go


create proc sp_rep_ahorros_2101(
       @s_date                datetime    =  null,
       @s_user                varchar(14) = 'isb_batch',
       @s_term                varchar(64) = 'isb_batch',
       @i_fin_mes             char(1)     = 'S',
       @i_fecha_desde         datetime    = null,       -- Fecha desde para consulta de Creditos y Debitos
       @i_fecha_hasta         datetime    = null,
       @i_fecha_sd	      datetime    = null,
	   @O_RETVAL		INT OUTPUT,
...
truncate table tmp_rep_liqaho_2101
truncate table tmp_rep_credeb_2101
--INICIALIZO VARIABLES
select @w_sp_name        = 'sp_rep_ahorros_2101',
       @w_tasa_base      = '0',
...
begin
   ...
   begin
	  ...
	  set @O_RETVAL=@w_error
	  set @O_RETMSG='EL RANGO DE FECHAS PARA CONSULTA DE CREDITOS Y DEBITOS, ES INCORRECTO'
	  return @O_RETVAL
      --goto ERROR1
   end
end
...
if exists (select 1 from anda_reporte..rep_cuadre_pasivas
           where  rf_prod_cobis = @w_prod_cobis and rf_fecha_carga=@i_fecha_desde and rf_cuenta not like '210150%' and rf_cuenta not like'2105%')
begin
...
insert into tmp_rep_liqaho_2101
...
declare cursor_ahorros_2101 cursor for
select la_cuenta,      la_cliente,     la_nombre,
       la_fecha_aper,  la_cta_banco,   la_saldo,
       la_oficina,     la_ced_ruc,     la_prod_banc,
       la_subtipo
from   tmp_rep_liqaho_2101

open cursor_ahorros_2101

fetch cursor_ahorros_2101 into  
...
begin   
   ...
   -- SALDO DIARIO
   select @w_saldo_ah = sd_saldo_disponible
   ...
   select @w_saldo_ah = @w_saldo_ah - @w_bloqueo_encaje 
   ...
   insert into tmp_rep_credeb_2101
  ...
  --- if @w_prod_banc in (1,2,3,6,7,8) 
   if @w_acepta_prod = 'S'
      ...
      select @w_cuenta       = '210135',
             @w_producto     = 'PAVISTA',
             @w_tasa_interes = 2.00,
             @w_sensible     = 'S'
   else
   if @w_prod_banc = 4
      select @w_cuenta   = '310310',
             @w_producto = 'MNP3OBL'
   else
   if @w_prod_banc = 5
      select @w_cuenta   = '310305',
             @w_producto = 'MNP3COM'
...
close cursor_ahorros_2101
deallocate cursor cursor_ahorros_2101
